...
class PipelineExecutor:
    """Executes sequential pipeline architectures where each agent feeds into the next."""
    ...
    def run_sync(
        ...
        """Synchronously executes pipeline architecture."""
        from app.execution.execution_engine import ExecutionEngine

        if architecture.topology != TopologyType.PIPELINE:
            logger.warning(
                f"PipelineExecutor called for topology '{architecture.topology.value}'. Routing to ExecutionEngine."
            )
    ...
    async def run(
        ...
        """Asynchronously executes pipeline architecture."""
